...
def clean(
    ...
    out: Path = typer.Option(
        None,
        "--out",
        help="Output parquet path. Defaults to models/clean.parquet",
    ),
    ...
    from civicflow.config import MODELS_DIR, CLEAN_PARQUET, RAW_CSV
...
def features(
    src: Path = typer.Option(
        None, "--in", help="Path to clean.parquet. Defaults to models/clean.parquet"
    ),
    out: Path = typer.Option(
        None, "--out", help="Output features.parquet. Defaults to models/features.parquet"
    ),
) -> None:
    import pandas as pd

    from civicflow.config import MODELS_DIR, CLEAN_PARQUET, FEATURES_PARQUET, TRAIN_CUTOFF_YEAR
...
def train(
    src: Path = typer.Option(
        None, "--features", help="Path to features.parquet. Defaults to models/features.parquet"
    ),
) -> None:
    import pandas as pd
...
def eval(
    ...
    from civicflow.config import MODELS_DIR, FEATURES_PARQUET
    ...
    if not cached and FEATURES_PARQUET.exists():
        ...
        try:
            ...
            raise typer.Exit(1)
        write_model_card(m, MODELS_DIR / "model_card.json")
