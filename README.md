...
**B2G SaaS** - Honolulu building-permit delay predictor and pre-submission requirements agent.
...
civicflow clean          # 432K rows parsed -> artifacts/clean.parquet
civicflow features       # contractor scores + feature matrix
civicflow train          # two-stage model -> artifacts/
...
civicflow/
  ...
  contractor.py     # name/license parsing + Credit Score (batch artifact)
...
artifacts/          # runtime models tracked; generated parquets remain gitignored
...
- **Render:** apply `render.yaml` to create one private Docker API service and one Next.js
  Web Service. Enter `OPENAI_API_KEY` and `CIVICFLOW_SITE_PASSWORD` when prompted. Render
  generates and shares the backend API key and session secret; no secrets use `NEXT_PUBLIC_*`.
...
- **Operations:** use `/live` for liveness and `/ready` for model readiness. Render must alert
  ...
  which also restores its versioned model artifacts.
